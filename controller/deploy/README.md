...
```shell
sealos build -t ghcr.io/sealos-apps/terminal/cluster/controller:latest -f Kubefile .
```

### How to run

```shell
sealos run ghcr.io/sealos-apps/terminal/cluster/controller:latest
```
...
- `SEALOS_HTTP_PORT`: HTTP port (auto-detected from sealos-config)
...
- Run Helm startup, liveness and readiness checks against the controller health endpoints
